...
다음은 ```test1```인덱스에 ```alias1``` 별칭을 부여하는 것 입니다.
...
이름 변경의 경우에는 1개의 요청에 ```remove```와 ```add```를 수행하시면 됩니다. 해당 동작은 원자성을 가지기 때문에 순간적으로 인덱스를 가리키지 못하는 것에 대한 걱정은 안하셔도 됩니다.
...
> 실수로 ```test```라는 인덱스를 추가한 경우
> ```test_2```라는 정상적인 인덱스를 만들고
...
별칭에 라우팅 정보를 추가할 수 도 있습니다. 필터링된 별칭에 함께 사용이 가능하며 불필요한 파편에 대한 동작을 피할 수 있습니다.

아래 명령은 ```test```인덱스에 ```alias1```이라는 별칭을 생성하는 것 입니다. 생성된 뒤 별칭에 대한 모든 동작은 ```1``` 파편에만 처리를 하도록 지정할 수 있습니다.
...
옵션을 통하여 색인 또는 검색할 라우팅 정보를 지정할 수 있습니다.
...
위 예제에도 나와있지만, 검색 라우팅 정보는 쉼표(,)를 이용하여 여러개를 지정할 수 있고 색인 라우팅 정보는 1개만 가능합니다.

만약 검색 라우팅이 설정된 별칭에 라우팅 파라미터를 다르게 사용하는 경우에는 두가지의 교집합이 되는 파편을 검색하게 됩니다. 아래 예제를 보시면 ```alias2```는 *1*, *2*를 검색 라우팅으로 설정했고, *2*, *3*을 검색 라우팅 파라미터로 전달하였으므로 실제 *2*번 파편에서 검색이 실행됩니다.
...
## 인덱스 생성시 별칭 지정 {#alias-index-creation}
